...
public class ComplaintService {
	...
	public Complaint saveForTest(final Complaint comp) {
		comp.setMoment(Calendar.getInstance().getTime());
		final FixUpTask fut = comp.getFixUpTask();
		final Complaint res = this.complaintRepository.save(comp);
		final Collection<Complaint> comps = fut.getComplaints();
		comps.add(res);
		fut.setComplaints(comps);
		return res;
	}
	...
	public Collection<Complaint> findByCustomerId(final Integer integer) {
		return this.complaintRepository.findByCustomerId(integer);
	}
